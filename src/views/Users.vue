  ...
  <v-sheet class="sc-page-sheet">
    ...
    >
      ...
      <template v-slot:item.groupid="{ item }">
        <sc-record-info :idx="item.groupid" store="groups/item" />
      </template>
            ...
            <v-row>
              ...
              <v-col cols="6">
                <v-select
                  ...
                  :items="$store.getters['groups/items']"
...
export default {
  ...
  methods: {
    generateNewPass() {
      this.pass = passUtils.generate(10);
    },
    edit(i) {
      this.item = i ? { ...i } : { ...DEF_ITEM };
      this.d_edit = true;
    },
    save() {
      if (!this.$refs.form.validate()) return;
      if (this.pass) {
        this.item.pass = this.$CryptoJS.enc.Base64.stringify(
          this.$CryptoJS.SHA256(this.pass)
        );
      }
  ...
  mounted() {
    ...
    if (!this.$store.getters["groups/isItems"]) {
