  ...
  <v-sheet class="radius-25">
    ...
    <v-row align="center" align-content="center">
      ...
      <v-select
        ...
        class="px-2 filter-select"
        clearable
        :label="$t('report.filterByService')"
      ></v-select>
      <v-select
...
export default {
  ...
  data() {
    return {
      ...
      coachs: [],
      sportclubs: [],
      filter: { club: null, workout: null, service: null, coach: null },
      headers: [
        {
          text: "#",
          excelIgnore: true,
        ...
        {
          text: this.$t("fields.scalarcount"),
          value: "scalarcount",
          align: "right",
        },
      ],
  ...
  methods: {
    filterData() {
      ...
      this.filtered = this.items.filter((i) => {
        ...
        if (tofilter) this.filteredTotal += i.scalarcount;
        return tofilter;
      });
    },
    loadData() {
      ...
      this.$api
        ...
        .then((data) => {
          ...
          this.items.forEach((e) => {
            ...
            if (!this.services.includes(e.service))
              this.services.push(e.service);
            if (e.workout && !this.workouts.includes(e.workout))
              this.workouts.push(e.workout);
            if (e.coach && !this.coachs.includes(e.coach))
              this.coachs.push(e.coach);

            this.allTotal += e.scalarcount;
          });
        });
    },
  },
